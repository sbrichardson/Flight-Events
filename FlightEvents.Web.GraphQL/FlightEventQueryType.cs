...
namespace FlightEvents.Web.GraphQL
{
    ...
    {
        public async Task<List<FlightPlanWaypoint>> GetCheckpointsAsync([Parent]FlightEvent @event, [Service]IFlightPlanStorage flightPlanStorage)
        {
            ...
            var flightPlan = await flightPlanStorage.GetFlightPlanAsync(@event.FlightPlanIds[0]);
